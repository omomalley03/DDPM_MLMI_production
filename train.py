"""
Training for DDPM (based on Ho 2020 paper).

Design Decisions:
- Adam optimizer (lr=2e-4)
- EMA with decay=0.9999
- Default steps is 650k (where we stopped our training)
- Save ckpts every 50k
"""
...
class EMA:
    ...
    def __init__(self, model, decay=0.9999):
        self.decay = decay
        self.shadow = copy.deepcopy(model) # make copy of model
        self.shadow.eval()
...
def train(
    ...
    # Model
    model = UNet( # this is what Ho used for CIFAR
        in_channels=3,
        base_channels=128,
    ...
    for step in range(start_step, total_steps):
        ...
        # Compute loss (Paper algorithm 1)
        ...
        # Update EMA
        # doesnt affect training -- just saved for future sampling
        ema.update(model)
