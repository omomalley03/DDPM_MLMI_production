...
def sample_progression(
    ...
    # frames[i]: tensor (n_samples, 3, H, W) — one per saved timestep
    # Stack to (n_samples, n_frames, 3, H, W), then flatten to (n_samples*n_frames, 3, H, W)
    # make_grid with nrow=n_frames gives one row per sample
    stacked = torch.stack(frames, dim=1)                          # (n_samples, n_frames, 3, H, W)
    stacked = stacked.view(-1, 3, image_size, image_size)         # (n_samples * n_frames, 3, H, W)
...
def sample_interpolate(
    ...
    n_samples=4,
    n_frames=10,
    output_dir="samples_cifar_interpolate",
    device="cuda",
    image_size=32,
):
    """Visualise the denoising process as a grid.
    ...
    print(f"Generating {n_samples} interpolations with ({n_frames} frames each)...")
    shape = (n_samples, 3, image_size, image_size)
    frames = diffusion.samples_interpolate(model, shape, interp_steps=n_frames)
    ...
    save_image(grid, out_path)
    print(f"Saved interpolation progression grid: {out_path}")
    print(f"  Grid layout: {n_samples} rows (samples) x {n_frames} columns (interp steps)")
